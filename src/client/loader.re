module type Config = {
  ...
  let make:
    data::data =>
    array int =>
    ReasonReact.componentSpec state state ReasonReact.noRetainedProps ReasonReact.noRetainedProps;
};
...
let fetchJson: string => (Js.Json.t => unit) => unit = [%bs.raw
  {| function (url, fn) {
  fetch(url).then(function (res) {return res.json()}).then(fn)
} |}
];
    ...
    didMount: fun {update} => {
      fetchJson
        ...
        (
          update (
            fun json _ =>
              switch (Config.data__from_json json) {
