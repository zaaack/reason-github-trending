...
let appState =
  ref {
    nextId: 1,
    todos: [{id: 0, text: "Add some things to do", completed: None}]
  };
...
Server.get
  "/api/todos" (fun _ _ _ => Server.json (todos__to_yojson (!appState).todos));
    ...
    fun _ body _ =>
      withParsedBody
        ...
        (
          fun id => {
            let todos =
              List.filter (fun item => item.id !== id) (!appState).todos;
...
Server.post
  ...
  Lwt.(
    fun _ body _ => {
      ...
      let todos =
        List.map
          (fun item => item.id === data.id ? data : item) (!appState).todos;
      appState := {...!appState, todos};
      Server.json (todos__to_yojson todos)
    }
  );
  ...
  (
    fun _ _ _ _ =>
      CoServer.respond_string
        status::`Not_found body::"Welcome to never land" ()
  );
